...
final class WPSCORE {
	...
	public function generate_sub_menu() {
		...
		add_submenu_page( '', '', '', 'manage_options', 'WPSCORE-dashboard', 'wpscore_dashboard_page_1_2_9' );
	 ...
	 *
	 * @return bool 1 if $nav_elt_1 > $nav_elt_2, -1 if $nav_elt_1 < $nav_elt_2, 0 if $nav_elt_1 === $nav_elt_2.
	 */
		...
		$output_logo = '
			<div class="row my-4">
				...
				<div class="col-sm-6 wpscript__header-social">
					<a href="https://twitter.com/wpscript" class="btn btn-default btn-sm mx-1 my-2" target="_blank"><img style="height:12px;" src="' . WPSCORE_TWITTER_LOGO_URL . '"> Follow us</a>
				</div>
			</div>';
		if ( ! $echo ) {
			return $output_logo;
	...
	public function init_installed_products() {
		...
		if ( ! is_object( $products_from_api ) ) {
			$products_from_api = json_decode( wp_json_encode( $products_from_api ) );
		}
	...
	public function init( $force = false ) {
		...
		$api_endpoint = str_replace( '/v2', '/v3', WPSCORE_API_URL . '/core/init' );
